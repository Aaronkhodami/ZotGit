var ZotMoovMenus = class
{
    ...
    _removeCustomMenuItemsFromWindow(win)
    {
        win.document.querySelectorAll('.' + this.menuitem_class).forEach(e =>
        {
            if (!e.id || e.id.startsWith('zotmoov-context-')) return;
            e.remove();
        });
    }
    ...
    refreshFromPrefs()
    {
        ...
        if(!Zotero.Prefs.get('extensions.zotmoov.enable_attach_dir', true))
        {
            this.hideAttachNewFile();
        } else
        {
            this.showAttachNewFile();
        }
        ...
        {
            if(!win.ZoteroPane) continue;
            this._removeCustomMenuItemsFromWindow(win);
            this._loadCMUFromPrefs(win);
        }
    }
    ...
    async importLastModifiedFile()
    {
        ...
        if (sync && typeof sync.getPreferredLocalDir == 'function') dst_path = sync.getPreferredLocalDir();
